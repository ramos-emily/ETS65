...
values 
    ('Incêndio, Explosão e Fumaça e Queda de Aeronave'),
...
-- Residencial Essencial; Residencial Prático e Residencial Total.
insert into seguro
...
insert into imovel
	(codigo_seguro, codigo_imovel_tipo, codigo_endereco, valor)
values
	(1, 2, 6, 500000.00);
...
-- Verifica resultado com informações complementares
select seguro.descricao as 'Descrição Seguro',
	   imovel_tipo.descricao as 'Tipo imóvel',
       concat(endereco.logradouro, ' - ', endereco.cidade,' - ', endereco.uf) as 'Endereço',
       concat('R$ ', format(imovel.valor, '2', 'de_DE')) as 'Valor'
from imovel
inner join seguro on imovel.codigo_seguro = seguro.codigo
inner join imovel_tipo on imovel.codigo_imovel_tipo = imovel_tipo.codigo
inner join endereco on imovel.codigo_endereco = endereco.codigo;



insert into seguro_pagamento
	(codigo_seguro, quantidade_parcelas, numero_parcela, valor_total, data_vencimento, valor_pago, data_pagamento)
values
	(1, 10, 1, 5000.00, '2023-08-10', 500.00, '2023-08-10'),
    (1, 10, 2, 5000.00, '2023-09-10', 515.00, '2023-09-25'),
    (1, 10, 3, 5000.00, '2023-10-10', 490.00, '2023-10-01'),
    (1, 10, 4, 5000.00, '2023-11-10', null, null),
    (1, 10, 5, 5000.00, '2023-12-10', null, null),
    (1, 10, 6, 5000.00, '2024-01-10', null, null),
    (1, 10, 7, 5000.00, '2024-02-10', null, null),
    (1, 10, 8, 5000.00, '2024-03-10', null, null),
    (1, 10, 9, 5000.00, '2024-04-10', null, null),
    (1, 10, 10, 5000.00, '2024-05-10', null, null);

-- Verifica resultado
select * from seguro_pagamento;



insert into seguro_cobertura
	(codigo_seguro, codigo_cobertura)
values
	(1, 1),
	(1, 2),
	(1, 4);
...
-- Verifica resultado
select * from seguro_cobertura;

-- Verifica resultado com informações complementares
select seguro.descricao,
       cobertura.descricao 
from seguro_cobertura
inner join seguro on seguro_cobertura.codigo_seguro = seguro.codigo
inner join cobertura on seguro_cobertura.codigo_cobertura = cobertura.codigo;




insert into seguro_servico
	(codigo_seguro, codigo_servico)
values
	(1, 1),
	(1, 2),
	(1, 3);
...
-- Verifica resultado
select * from seguro_servico;

-- Verifica resultado com informações complementares
select seguro.descricao,
       servico.descricao 
from seguro_servico
inner join seguro on seguro_servico.codigo_seguro = seguro.codigo
inner join servico on seguro_servico.codigo_servico = servico.codigo;
...
select seguro.descricao, 
	   cobertura.descricao
from seguro
inner join seguro_cobertura on seguro.codigo = seguro_cobertura.codigo_seguro
inner join cobertura on seguro_cobertura.codigo_cobertura = cobertura.codigo;


-- Seguro e seus serviços
select seguro.descricao, 
       servico.descricao
from seguro
inner join seguro_servico on seguro.codigo = seguro_servico.codigo_seguro
inner join servico on seguro_servico.codigo_servico = servico.codigo;
